...
/skills - используй для добавления нового навыка для проекта

/projects - используй для того, чтобы посмотреть все проекты
...
def link_project(message, data):
    ...
    bot.send_message(message.chat.id, "Введите текущий статус проекта", reply_markup=gen_markup([x[0] for x in rows]))
    bot.register_next_step_handler(message, callback_project, data=data)

def callback_project(message, data):
    status_id = manager.get_status_id(message.text)
...
def skill_project(message, projects):
    ...
    if project_name not in projects:
        bot.send_message(message.chat.id, 'У тебя нет такого проекта, попробуй еще раз!) Выбери проект для которого нужно выбрать навык', reply_markup=gen_inline_markup(projects))
        bot.register_next_step_handler(message, skill_project, projects=projects)
        return
    bot.register_next_step_handler(message, choose_skill, project_name=project_name)

def choose_skill(message, project_name):
    skills = [x[1] for x in manager.get_skills()]
    bot.send_message(message.chat.id, 'Выбери навык', reply_markup=gen_inline_markup(skills))
...
def set_skill(message, project_name, skills):
    ...
    if skill not in skills:
        bot.send_message(message.chat.id, 'Видимо, ты выбрал навык. не из спика, попробуй ее раз!) Выбери навык', reply_markup=gen_inline_markup(skills))
        bot.register_next_step_handler(message, set_skill, project_name=project_name, skills=skills)
        return
    manager.insert_skill(user_id, project_name, skill )
    bot.send_message(message.chat.id, 'Навык {skill} добавлен проекту {project_name}!')
...
def delete_handler(message):
    ...
    projects = manager.get_projects(user_id)
    text = "\n".join([f"Project name:{x[2]} \nLink:{x[4]}\n" for x in projects])
    bot.send_message(message.chat.id, text, reply_markup=gen_markup([x[2] for x in projects]))
...
def delete_project(message, projects):
    ...
    bot.send_message(message.chat.id, 'Проект {project} удален!')
